...
def extract_enumerations(ttl_file):
    ...
    for enum_class in enum_classes:
        ...
        if values:
            # Get enum class info
            enum_label = g.value(enum_class, RDFS.label)
            enum_comment = g.value(enum_class, RDFS.comment)
...
def generate_enumeration_html(enumerations):
    ...
    for enum_uri, enum_data in enumerations.items():
        ...
        html_parts.append('<thead><tr><th>Value</th><th>Label</th><th>Description</th></tr></thead>')
